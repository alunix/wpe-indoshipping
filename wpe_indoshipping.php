...
wp_enqueue_style('indoshipping_css', plugin_dir_url(__FILE__) . 'assets/indoshipping.css');
wp_enqueue_script('indoshipping_js', plugin_dir_url(__FILE__) . 'assets/indoshipping.js', array('jquery'));
...
function wpe_load_province_html() {
	...
	$weight = $totalweight;
	$html = '';
	$html.= '<td><select name="sel_propinsi" id="sel_propinsi" onchange="wpei_get_kota(this.value);">' . "\n";
...
class wpe_indoshipping {
	...
	function getQuote() {
		...
		if ($country == 'ID'):
			...
			return array($this->name . ' For only in Indonesia' => 0);
		endif;
	}
